      ...
      <View pointerEvents={'box-none'} style={[styles.headerContainer, stylesHeader]}>
        <View pointerEvents={'box-none'} style={[styles.styleTitle, styleMainText]}>
          {mainText ? <Text
            ...
            style={[styles.headerText, stylesHeaderText]}>{mainText || ''}</Text> : null}
          {props.children && <View style={{ flex: 1, width: '100%', justifyContent: 'center', paddingTop: 15 }}>
            {props.children}
          </View>}
        </View>
        <TouchableOpacity
          ...
          hitSlop={hitSlop}
          onPress={leftAction}>
          {imageLeft ? <Image resizeMode='cover' source={imageLeft} /> : (
            leftComponent ? (
              leftComponent
            ) : null
          )}
          {leftText ? <Text style={[styles.secondaryHeaderText, styles.cancelText, stylesTextLeft]}>{leftText}</Text> : null}
        </TouchableOpacity>
        <View pointerEvents={'box-none'} style={{ flexDirection: 'row', alignItems: 'center' }}>
          {actionRight.map((value: any, index: number) =>
            <TouchableOpacity key={index} hitSlop={hitSlop} style={[styles.secondaryHeaderImage, index > 0 ? { marginLeft: 20 } : {}, value.styleTouchable || {}]} onPress={value.action}>
